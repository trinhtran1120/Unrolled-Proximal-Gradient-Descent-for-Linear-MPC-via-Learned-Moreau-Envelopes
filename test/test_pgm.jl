...
@testset "Test proximal gradient method" begin
    ...
    W = fill(0.0, problem.nu, problem.N)
    project = projection(problem, problem.x0, W)
    U_proj = project(problem.x0, W)
    X_proj = rollout(problem, problem.x0, U_proj)

    for k in 1:problem.N
        @test all(problem.umin .- 1e-6 .<= U_proj[:, k])
        @test all(U_proj[:, k] .<= problem.umax .+ 1e-6)
        @test all(problem.xmin .- 1e-6 .<= X_proj[:, k + 1])
        @test all(X_proj[:, k + 1] .<= problem.xmax .+ 1e-6)
    end

    result = PGM(problem, problem.x0; rho=0.001, max_iter=5000, tol=5e-4)
    ...
    # println(abs(J_pgm - J_opt) / abs(J_opt))
    @test abs(J_pgm - J_opt) / abs(J_opt) <= 1e-1
